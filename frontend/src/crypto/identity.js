...
export function userIdForPublicKey(identityPk) {
...
export function kexKeyBindingBytes({ userId, identityPk, kexPk }) {
  const identityKey = typeof identityPk === "string" ? base64ToBytes(identityPk) : identityPk;
  const exchangeKey = typeof kexPk === "string" ? base64ToBytes(kexPk) : kexPk;
...
export function signKexKeyBinding(identity) {
 ...
 */
export function verifyKexKeyBinding({ userId, identityPk, kexPk, kexKeySignature }) {
  try {
    const identityKey = typeof identityPk === "string" ? base64ToBytes(identityPk) : identityPk;
    const signature = typeof kexKeySignature === "string"
      ? base64ToBytes(kexKeySignature)
      : kexKeySignature;
...
export function safetyFingerprint({ userId, identityPk, peerUserId, peerIdentityPk }) {
  const first = { id: userId, key: typeof identityPk === "string" ? base64ToBytes(identityPk) : identityPk };
  const second = {
    id: peerUserId,
    key: typeof peerIdentityPk === "string" ? base64ToBytes(peerIdentityPk) : peerIdentityPk,
  };
...
export function signChallenge(identity, nonceBytes) {
